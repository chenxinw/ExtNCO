...
def load_my_dataset(input_tensor, k_sparse, device, start_node=None):
    data_list = []
    for instance in input_tensor:
        instance = instance.to(device)
        data, distances = gen_pyg_data(instance, k_sparse=k_sparse, start_node=start_node)
        data_list.append((data, distances))
    return data_list