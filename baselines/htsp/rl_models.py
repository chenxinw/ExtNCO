...
class IMPALACNN(nn.ModuleDict):
    ...
    def __init__(self, input_channels, output_dim, depths=None, input_shape=None):
        ...
        for num_ch in depths:
            feats_convs = []
            feats_convs.append(nn.Conv2d(in_channels=input_channels, out_channels=num_ch,
            ...
            for i in range(2):
                ...
                resnet_block.append(nn.Conv2d(in_channels=input_channels, out_channels=num_ch,
                                              kernel_size=3, stride=1, padding=1))
                resnet_block.append(nn.ReLU())
                resnet_block.append(nn.Conv2d(in_channels=input_channels, out_channels=num_ch,
                                              kernel_size=3, stride=1, padding=1))
    ...
    def _get_flatten_dim(self, input_shape):
        input_sample = torch.empty([1, self.input_channels, input_shape[0].int().item(), input_shape[1].int().item()])
        x = input_sample
...
class IMPALAEncoder(nn.ModuleDict):
    ...
    def __init__(self, input_dim, embedding_dim, bev_range=None, bev_pixel_size=None, depths=None, ):
        ...
        self.input_transform = nn.Sequential(
                nn.Conv1d(self.input_dim + 4, self.input_channels, kernel_size=1, stride=1, padding=0, bias=False, ),
                nn.InstanceNorm1d(self.input_channels, eps=1e-3, momentum=0.01, affine=True),
                nn.ReLU(inplace=True))
        self.cnn = IMPALACNN(self.input_channels, self.embedding_dim, depths, bev_grid_shape)
    ...
    def forward(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        batch_idxs = (torch.arange(0, B).view(1, B).repeat(N, 1).t().flatten().to(batch.device))
        ...
        bev_merge_coords = points[:, 0].int() * bev_scale_xy + bev_coords[:, 0] * bev_scale_y + bev_coords[:, 1]
        bev_unq_coords, bev_unq_inv, bev_unq_cnt = torch.unique(
                bev_merge_coords, return_inverse=True, return_counts=True, dim=0)
        bev_f_center = points_xy - ((bev_coords.to(points_xy.dtype) + 0.5) * bev_pixel_size + bev_range[[0, 1]])
        ...
        mvf_input = torch.cat([points_xy, bev_f_center, bev_f_cluster, extra_feat], dim=1).contiguous()
        # [B * N, input_dim + 4]
        ...
        pixel_coords = torch.stack((
                ...
                torch.div(bev_unq_coords % bev_scale_xy, bev_scale_y, rounding_mode="floor"),
                torch.div(bev_unq_coords % bev_scale_y, 1, rounding_mode="floor"),
                bev_unq_coords % 1,), dim=1)
        pixel_coords = pixel_coords[:, [0, 3, 2, 1]]

        # forward image
        batch_bev_features = []
        for batch_idx in range(B):
            feature = torch.zeros(H, bev_scale_xy.int().item(), dtype=bev_fea_in.dtype, device=bev_fea_in.device)

            batch_mask = pixel_coords[:, 0] == batch_idx
            this_coords = pixel_coords[batch_mask, :]
            indices = this_coords[:, 1] + this_coords[:, 2] * bev_grid_shape[0] + this_coords[:, 3]
...
class ActorPPO(nn.Module):
    ...
    def __init__(self, state_dim, mid_dim, action_dim, init_a_std_log=-0.5):
        super().__init__()
        nn_middle = nn.Sequential(nn.Linear(state_dim, mid_dim),
                                  nn.ReLU(),
        ...
        self.a_std_log = nn.Parameter(torch.ones((1, action_dim)).mul_(init_a_std_log), requires_grad=True)
    ...
    def reset_parameter(self):
        for name, module in self.net.named_modules():
            if isinstance(module, torch.nn.Linear):
                layer_norm(module)
        last_layer = self.net[-1]  # rescale last layer
